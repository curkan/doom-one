...
local config = {
	...
	plugins = {
		...
		barbar = if_nil(vim.g.doom_one_plugin_barbar, true),
		telescope = if_nil(vim.g.doom_one_plugin_telescope, true),
...
doom_one.set_colorscheme = function()
	...
	set_hl("Visual", { bg = palette.grey })
	set_hl("VisualBold", { bg = palette.grey, bold = true })
	...
	----------------------------------------
	set_hl("Search", { bg = palette.grey, fg = "fg" })
	...
	set_hl("Parameter", { fg = dark_theme and palette.maroon or palette.base4, bold = true })
	set_hl("White", { fg = dark_theme and palette.white or palette.base4, bold = false })
	...
	set_hl("Function", { fg = palette.yellow })
	...
	if config.ui.enable_treesitter then
		...
		set_hl("@argument", { link = "Attribute" })
		...
		set_hl("@variable", { link = "White" })
		set_hl("@variable.builtin", { link = "VariableBuiltin" })
		set_hl("@variable.parameter", { link = "CommentBold" })
	...
	if config.plugins.nvim_tree then
		set_hl("NvimTreeFolderName", { fg = dark_theme and palette.yellow or palette.base8, bold = true })
